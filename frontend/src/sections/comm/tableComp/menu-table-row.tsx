...
type MenuTableRowProps = {
  ...
  selected: boolean;
  onSelectRow: () => void;
  onDeleteUser: (menu: MenuProps) => void;  // 부모에서 전달된 삭제 함수
  onEditUser: (menu: MenuProps) => void;  // 부모에서 전달된 삭제 함수
};

export function MenuTableRow({ row, selected, onSelectRow, onDeleteUser, onEditUser}: MenuTableRowProps) {
  ...
  const handleEdit = useCallback(() => {
    onEditUser(row);  // 부모의 onDeleteUser 호출하며 row 데이터를 전달
    handleClosePopover();
    ...
    <>
      <TableRow hover tabIndex={-1} role="checkbox" selected={selected}>
        ...
        */}
        ...
        <TableCell>{row.insert_id}</TableCell>

        <TableCell>{row.insert_date}</TableCell>

        <TableCell>{row.update_id}</TableCell>
